...
class OCGatherEnergyCorrFac(tf.keras.layers.Layer):
    ...
    def call(self, pred_sid, pred_corr_factor, rechit_energy, row_splits=None):
        ...
        unique_segments = pred_sid_p1[:, 0] + (
            tf.reduce_max(pred_sid_p1) + 1
        ) * tf.ragged.row_splits_to_segment_ids(row_splits, out_type=tf.int32)
        pred_shower_energy = tf.math.unsorted_segment_sum(
            rechit_energy[:, 0] * pred_corr_factor[:, 0],
            unique_segments,
...
class OCGatherEnergyCorrFac3(tf.keras.layers.Layer):
    ...
    def call(
        ...
        pred_beta,
        is_track=None,
        return_track_if_possible=False,
        row_splits=None,
        ...
        is_track = tf.reshape(
            tf.gather(is_track, no_noise_idx), (-1, 1)
        )  # Shape [N_filtered, 1]
        rechit_energy = tf.reshape(
            tf.gather(rechit_energy, no_noise_idx), (-1, 1)
        )  # Shape [N_filtered, 1]
        ...
        pred_shower_tracks = tf.math.unsorted_segment_sum(
        ...
        pred_shower_hits = tf.math.unsorted_segment_sum(
        ...
        if return_tracks_where_possible:
            pred_energy = tf.where(
                pred_energy_tracks != 0.0, pred_energy_tracks, pred_energy_hits
            )
            return pred_energy[:, tf.newaxis]
        elif return_tracks:
...
def pandora_layer(filtered_truth_df):
    pandora_idx = filtered_truth_df.truthHitAssignementIdx_pandora
    return pandora_idx, pandora_idx
...
def process_endcap2(
    ...
    # if filtered_truth_df is not None:
    #     pred_sid, _, alpha_idx, _, ncond = pandora_layer(filtered_truth_df)
    ...
    if "pred_energy_high_quantile" in predictions_dict.keys():
        processed_pred_dict["pred_energy_unc"] = 0.5 * (
            predictions_dict["pred_energy_high_quantile"]
            - predictions_dict["pred_energy_low_quantile"]
        )
        processed_pred_dict.update(predictions_dict)
        processed_pred_dict.pop("pred_beta")
        processed_pred_dict["pred_id"] = np.argmax(
            processed_pred_dict["pred_id"], axis=1
        )
...
def process_endcap(
    hits2showers_layer, energy_gather_layer, features_dict, predictions_dict
):
    pred_sid, _, alpha_idx, _, ncond = hits2showers_layer(
        predictions_dict["pred_ccoords"],
        predictions_dict["pred_beta"],
