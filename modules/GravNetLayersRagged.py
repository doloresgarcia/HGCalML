...
def select_threshold_with_backgather(score, threshold, row_splits):
    ...
    for i in tf.range(tf.shape(row_splits)[0] - 1):
        ...
        minidx = keeprange[minidxlocal_rs][
            0
    ...
    sel = tf.expand_dims(
        tf.concat(allkeepidxs, axis=0), axis=1
    )  # standard tf index format
    newrs = tf.concat(newrs, axis=0)

    # sanity check
    selglidxs = SelectWithDefault(sel, tf.expand_dims(glidxs, axis=1), 0)[:, :, 0]
    mess = (
        "select_threshold_with_backgather: assert indices "
        + str(selglidxs.shape)
        + " vs "
        + str(sel.shape)
    )
...
class CreateMask(tf.keras.layers.Layer):
    ...
    def get_config(self):
        ...
        return dict(
            ...
            + list({"threshold": self.threshold, "invert": self.invert}.items())
        )
...
class ShiftDistance(tf.keras.layers.Layer):
    ...
    def __init__(self, shift=0.1, **kwargs):
        """
        Inputs:
        - Square of distance
...
class SplitOffTracks(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        rnottrack = ristrack == 0.0
        ristrack = ristrack != 0.0
...
class MaskTracksAsNoise(tf.keras.layers.Layer):
    def __init__(self, active=True, maskidx=-1, **kwargs):
        """
        Inputs:
        - truth index
        - track_charge
    ...
    def get_config(self):
        ...
        return dict(
            list(base_config.items())
            + list({"active": self.active, "maskidx": self.maskidx}.items())
        )
...
class CondensateToIdxs(LayerWithMetrics):
    def __init__(
        self, t_d, t_b, active=True, keepnoise=False, return_thresholds=False, **kwargs
    ):
        """
    ...
    def call(self, inputs):
        ...
        if rs.shape[0] == None:
            ...
            if self.return_thresholds:
                if self.return_full_assignment:
                    return pred_sid, asso, alpha, ncond, self.dyn_t_d, self.dyn_t_b
        ...
        if self.active:
            ...
            d = d * t_d
            t_b = tf.abs(self.dyn_t_b).numpy()[
                0
            ]  # abs is safety guard as the constraint does not seem to be inf

            pred_sid, asso_idx, alpha, _, ncond = ba_cond(
                ccoords,
                beta,
                row_splits=rs,
                beta_threshold=t_b,
                ...
                for i in tf.range(rs.shape[0] - 1):
                    with tf.control_dependencies([pred_sid]):
                        ...
                        nseg_a = tf.reduce_sum(
                            tf.cast(tf.unique(tf.squeeze(check))[0], dtype="int32") * 0
                            + 1
                        )
        ...
        if self.return_thresholds:
            ...
            if self.return_full_assignment:
                return pred_sid, asso_idx, alpha, ncond, self.dyn_t_d, self.dyn_t_b
            else:
                return pred_sid, asso_idx, alpha, self.dyn_t_d, self.dyn_t_b
...
class CleanCondensations(LayerWithMetrics):
    ...
    def get_config(self):
        ...
        return dict(
            list(base_config.items()) + list({"threshold": self.threshold}.items())
        )
    ...
    def call(self, inputs):
        ...
        pred_sid = tf.where(score[:, 0] < self.threshold, -1, pred_sid[:, 0])[
            :, tf.newaxis
        ]
...
class CreateIndexFromMajority(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        return CreateIndexFromMajority.raw_call(
            inputs, self.min_threshold, self.ignore, self.assign_else
        )
...
class ElementScaling(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.scales = self.add_weight(
            name="scales", shape=shape, initializer="ones", trainable=True
        )
...
class GooeyBatchNorm(LayerWithMetrics):
    def __init__(
        ...
        variance_only=False,
        soft_mean=False,
        soften_update: float = 0.0,
        soft_mean_hardness=3.0,
        soft_mean_turn_on=6.0,
        ...
        **kwargs,
        ...
        assert viscosity >= 0 and viscosity <= 1.0
        assert fluidity_decay >= 0 and fluidity_decay <= 1.0
        ...
        if soften_update > 0:
            print(
                self.name,
                "has been configured for soften_update. Function not implemented yet. Will have no effect.",
            )

    def get_config(self):
        config = {
            "viscosity": self.viscosity_init,
    ...
    def build(self, input_shapes):
        ...
        self.mean = self.add_weight(
            name="mean", shape=shape, initializer="zeros", trainable=self.learn
        )

        self.variance = self.add_weight(
            name="variance", shape=shape, initializer="ones", trainable=self.learn
        )

        self.viscosity = tf.Variable(
            initial_value=self.viscosity_init,
            name="viscosity",
            trainable=False,
            dtype="float32",
        )
    ...
    def _calc_update(self, old, new, training, ismean):
        delta = new - old
        update = old + (1.0 - self.viscosity) * delta
    ...
    def call(self, inputs, training=None):
        ...
        if self.trainable and not self.learn:
            ...
            self.trainable and self.learn
        ):  # trainable is anyway given by applying the loss or not
            currentmean = tf.reduce_mean(x, axis=0, keepdims=True)
            currentmean = tf.stop_gradient(currentmean)
            currentmean = tf.where(
                tf.math.is_finite(currentmean), currentmean, self.mean
            ...
            newvar = tf.where(
                tf.math.is_finite(newvar), newvar, self.variance
            )  # protect against empty batches
...
class ScaledGooeyBatchNorm(GooeyBatchNorm):
    ...
    def build(self, input_shapes):
        ...
        self.bias = self.add_weight(
            name="bias", shape=shape, initializer="zeros", trainable=self.trainable
        )

        self.gamma = self.add_weight(
            name="gamma", shape=shape, initializer="ones", trainable=self.trainable
        )
...
class ScaledGooeyBatchNorm2(tf.keras.layers.Layer):
    ...
    def build(self, input_shapes):
        ...
        self.mean = self.add_weight(
            name="mean", shape=shape, initializer="zeros", trainable=False
        )
        self.den = self.add_weight(
            name="den", shape=shape, initializer="ones", trainable=False
        )
        self.viscosity = tf.Variable(
            initial_value=self.viscosity_init,
            name="viscosity",
    ...
    def call(self, inputs, training=None):
        if isinstance(inputs, list):
            x_in, cond = inputs
            cond = tf.where(
                cond > 0.5, tf.ones_like(cond), 0.0
...
class ConditionalScaledGooeyBatchNorm(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        ...
        raise ValueError(
            "problems with weight saving, use two ScaledGooeyBatchNorm2 layers and invert condition on one."
        )
...
class ProcessFeatures(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if not self.newformat:
            ...
            fdict["recHitX"] /= 100.0
            fdict["recHitY"] /= 100.0
            fdict["recHitZ"] = (tf.abs(fdict["recHitZ"]) - 400) / 100.0
            fdict["recHitEta"] = tf.abs(fdict["recHitEta"])
            fdict["recHitTheta"] = 2.0 * tf.math.atan(tf.exp(-fdict["recHitEta"]))
            fdict["recHitTime"] = (
                tf.nn.relu(fdict["recHitTime"]) / 10.0
            )  # remove -1 default
            allf = []
            for k in fdict:
                allf.append(fdict[k])
...
class ManualCoordTransform(tf.keras.layers.Layer):
    ...
    def _calc_abseta(self, x, y, z):
        ...
        return -1 * tf.math.log(r / tf.abs(z + 1e-3) / 2.0)
...
class DirectedGraphBuilder(tf.keras.layers.Layer):
    def __init__(self, strength=1.0, cutoff=1.0, **kwargs):
        """
    ...
    def get_config(self):
        config = {"strength": self.strength, "cutoff": self.cutoff}
...
class NeighbourApproxPCA(tf.keras.layers.Layer):
    def __init__(self, hidden_nodes=[32, 32, 32], **kwargs):
        ...
        for i in range(len(hidden_nodes)):
            with tf.name_scope(self.name + "/1/" + str(i)):
                self.hidden_dense.append(
                    tf.keras.layers.Dense(hidden_nodes[i], activation="elu")
                )
        ...
        print(
            "NeighbourApproxPCA: Warning. This layer is still very sensitive to the input normalisations and the learning rates."
        )
...
class ApproxPCA(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(
            "This layer uses the pretrained PCA approximation layers found in `pca/pretrained`"
        )
...
class CreateGlobalIndices(tf.keras.layers.Layer):
    ...
    def compute_output_signature(self, input_signature):
        ...
        return [
            tf.TensorSpec(dtype=tf.int32, shape=output_shape[i])
...
class WeightedNeighbourMeans(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        nweights = SelectWithDefault(nidx, weights, 0.0)[:, :, 0]  # remove last 1 dim

        if self.distweight:
            nweights *= tf.exp(-10.0 * dist)  # needs to be before log
...
class WeightFeatures(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        weight = (
            self.weight_dense(wfeat) / 0.761594156
...
class RecalcDistances(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        ncoords = SelectWithDefault(nidx, coords, 0.0)  # V x K x C
...
class MultiBackScatterOrGather(tf.keras.layers.Layer):
    ...
    @staticmethod
    def raw_call(x, scatters, default=0.0):
        xin = x
        for k in range(len(scatters)):
            l = len(scatters) - k - 1
            if isinstance(scatters[l], list):
                ...
                x = SelectFromIndices.raw_call(
                    tf.cast(scatters[l], tf.int32), [x], [[-1] + list(x.shape[1:])]
                )
...
class WarpedSpaceKNN(tf.keras.layers.Layer):
    def __init__(self, K: int, radius: float = -1.0, **kwargs):
        """
    ...
    def get_config(self):
        config = {"K": self.K, "radius": self.radius}
    ...
    def raw_call(coordinates, row_splits, warp, K, radius):
        idx, dist = SelectModKnn(
            K + 1, coordinates, warp, row_splits, max_radius=radius, tf_compatible=False
        )
    ...
    def call(self, inputs):
        ...
        return WarpedSpaceKNN.raw_call(
            coordinates, row_splits, warp, self.K, self.radius
        )
...
class NoiseFilter(LayerWithMetrics):
    def __init__(
        self, threshold=0.1, print_reduction=False, return_backscatter=True, **kwargs
    ):
    ...
    def call(self, inputs):
        ...
        if self.print_reduction:
            print(
                ...
                int(newrs[-1]),
                ": ",
                float(row_splits[-1]) / float(newrs[-1]),
            )

        self.add_prompt_metric(
            tf.cast(newrs[-1], dtype="float32")
            / tf.cast(row_splits[-1], dtype="float32"),
            self.name + "_reduction",
        )
...
class EdgeCreator(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.addself:
            edges = selffeat - SelectWithDefault(inputs[0][:, :], inputs[1], -1.0)
        else:
            edges = selffeat - SelectWithDefault(inputs[0][:, 1:], inputs[1], -1.0)
        return edges
...
class DampenGradient(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        return self.strength * tf.stop_gradient(inputs) + (1.0 - self.strength) * inputs


class GroupScoreFromEdgeScores(tf.keras.layers.Layer):
    def __init__(self, den_offset=0.0, **kwargs):
        """
        Input:
    ...
    def get_config(self):
        ...
        return dict(
            list(base_config.items()) + list({"den_offset": self.den_offset}.items())
        )
    ...
    def call(self, inputs):
        if len(inputs) == 3:
            ...
            energy = SelectWithDefault(nidx, energy, 0.0)
            energy = tf.reduce_sum(energy, axis=1)
            eweight = (1.0 + tf.sqrt(energy))[:, 0]  # V
        else:
            score, nidx = inputs
            eweight = 1.0
        # take mean
        ...
        n_neigh = tf.cast(n_neigh, dtype="float32") - 1.0
...
class NeighbourGroups(LayerWithMetrics):
    def __init__(
        ...
        **kwargs,
        ...
        if threshold is None:
            ...
            assert (
                purity_min_target is None and efficiency_min_target is None
    ...
    def get_config(self):
        config = {
            "purity_min_target": self.purity_min_target,
    ...
    def _update_threshold(self, purity, efficiency, training=None):
        ...
        update = (1.0 + self.thresh_viscosity * add) * self.threshold
        update = tf.clip_by_value(update, 1e-3, 1.0 - 1e-3)
    ...
    def call(self, inputs, training=None):
        ...
        score = tf.clip_by_value(score, 0.0, 1.0)
        ...
        if row_splits.shape[0] is None:
            ...
            for i in range(tf.shape(row_splits)[0] - 1):
                a = tf.argsort(
                    score[row_splits[i] : row_splits[i + 1]],
                    axis=0,
            ...
            # rewrite Localgroup
            rs, dirnidx, sel, ggather = LocalGroup(
                nidxs,
                hierarchy_idxs,
                score,
                row_splits,
                score_threshold=self.threshold.numpy(),
            )  # force eager
...
class AccumulateNeighbours(tf.keras.layers.Layer):
    ...
    def get_min(self, ndix, feat):
        out, _ = AccumulateLinKnn(
            tf.zeros_like(ndix, dtype="float32") + 1.0, -feat, ndix, mean_and_max=True
        )
    ...
    def call(self, inputs, training=None):
        ...
        if len(inputs) == 2:
            feat, ndix = inputs
            w = tf.cast(tf.zeros_like(ndix), dtype="float32") + 1.0
        else:
            feat, ndix, w = inputs
            w = SelectWithDefault(ndix, w, 0.0)[:, :, 0]
        ...
        if self.mode == "mean" or self.mode == "meanmax":
            out, _ = AccumulateLinKnnSumw(
                w, feat, ndix, mean_and_max=self.mode == "meanmax"
            )
            return out
        if self.mode == "gnlike":
            out, _ = AccumulateLinKnn(w, feat, ndix)
        ...
        if self.mode == "sum":
            ...
            return tf.reshape(out, [-1, feat.shape[1]])
        if self.mode == "max":
            out, _ = AccumulateLinKnn(w, feat, ndix, mean_and_max=True)
            out = out[:, feat.shape[1] :]
            return tf.reshape(out, [-1, feat.shape[1]])
        if self.mode == "min":
            return self.get_min(ndix, feat)
        if self.mode == "minmeanmax":
            meanmax, _ = AccumulateLinKnnSumw(w, feat, ndix, mean_and_max=True)
...
class SoftPixelCNN(tf.keras.layers.Layer):
        ...
        **kwargs,
        ...
        with tf.name_scope(self.name + "/1/"):
            self.length_scale = tf.Variable(
                initial_value=1.0, trainable=False, dtype="float32"
            )
    ...
    def get_config(self):
        config = {
            "length_scale_momentum": self.length_scale_momentum,
            "mode": self.mode,
    ...
    def call(self, inputs, training=None):
        ...
        if self.trainable:
            tf.keras.backend.update(
                ...
                tf.keras.backend.in_train_phase(
                    self.length_scale * (1.0 - self.length_scale_momentum)
                    + self.length_scale_momentum
                    * 2.0
                    * tf.reduce_mean(tf.sqrt(distsq + 1e-6)),
                    self.length_scale,
                    training=training,
                ),
            )
        ...
        for o in self.offsets:
            ...
            f, _ = AccumulateKnn(
                self.length_scale * distancesq, features, neighbour_indices
            )  # V x F
            f = f[:, : self.nfeat]
...
class SelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, input):
        att = 2.0 * self.att_dense(input)
        return input * att


class MultiAttentionGravNetAdd(tf.keras.layers.Layer):
    def __init__(self, n_attention_kernels: int, **kwargs):
        """
    ...
    def call(self, inputs):
        ...
        for di in range(len(self.kernel_coord_dense)):
            ...
            othercoord = SelectWithDefault(nidx, coord, 0.0)  # V x K x C
            dist = tf.reduce_sum((othercoord - refcoord) ** 2, axis=2)
            acc, _ = AccumulateKnn(10.0 * dist, passfeat, nidx, mean_and_max=False)
...
class DynamicDistanceMessagePassing(tf.keras.layers.Layer):
    ...
    def __init__(self, n_feature_transformation, **kwargs):
        ...
        for i in range(1, len(self.dist_mod_dense)):
            with tf.name_scope(self.name + "/5/" + str(i)):
                self.dist_mod_dense[i].build(
                    (
                        input_shape[0],
                        input_shape[1] + self.n_feature_transformation[i - 1] * 2,
                    )
                )
    ...
    def create_output_features(self, x, neighbour_indices, distancesq):
        ...
        for i in range(len(self.n_feature_transformation)):
            if i == 0:
                scale = 10.0 * self.dist_mod_dense[0](x)
            else:
                scale = 10.0 * self.dist_mod_dense[i](tf.concat([x, features], axis=-1))
            distancesq *= scale
...
class MessagePassing(tf.keras.layers.Layer):
    ...
    def collect_neighbours(self, features, neighbour_indices):
        ...
        f, _ = AccumulateLinKnn(
            tf.cast(neighbour_indices * 0, tf.float32) + 1.0,
            features,
            neighbour_indices,
        )
        return f
...
class DistanceWeightedAttentionMP(DistanceWeightedMessagePassing):
    ...
    def build(self, input_shapes):
        ...
        for i in range(1, len(self.feature_tranformation_dense)):
            total_feat = (
                2 * self.n_feature_transformation[i - 1] + self.K * self.pos_emb.units
            )
            with tf.name_scope(self.name + "/6/" + str(i)):
    ...
    def call(self, inputs):
        ...
        pos_emb = SelectWithDefault(nidx, pos_emb, -2.0)
        ...
        for i in range(len(self.n_feature_transformation)):
            ...
            w = self.attention_dense[i](emb) * tf.exp(-10.0 * distancesq)
...
class EdgeConvStatic(tf.keras.layers.Layer):
    ...
    def __init__(
        self, n_feature_transformation, select_neighbours=True, add_mean=False, **kwargs
    ):
        ...
        for i in range(len(self.n_feature_transformation)):
            with tf.name_scope(self.name + "/5/" + str(i)):
                self.feature_tranformation_dense.append(
                    tf.keras.layers.Dense(
                        self.n_feature_transformation[i], activation="elu"
                    )
                )
    ...
    def call(self, inputs):
        ...
        if self.select_neighbours:
            ...
            neighfeat = tf.where(
                tf.expand_dims(nidx, axis=2) < 0, 0.0, neighfeat
            )  # zero pad
...
class SingleLocalGravNetAttention(tf.keras.layers.Layer):
    def __init__(self, n_keys: int, n_values: int, n_pre_keys: int = -1, **kwargs):
        """
        One head
    ...
    def call(self, inputs):
        ...
        if self.n_pre_keys > 0:
            ...
            n_keys = SelectWithDefault(nidx, keys, 0.0)  # [V x K x F]
...
class LocalGravNetAttention(tf.keras.layers.Layer):
    ...
    def __init__(
        self, n_heads: int, n_keys: int, n_values: int, n_pre_keys: int = -1, **kwargs
    ):
        ...
        for i in range(self.n_heads):
            with tf.name_scope(self.name + "/1/" + str(i)):
                self.heads.append(
                    SingleLocalGravNetAttention(n_keys, n_values, n_pre_keys=n_pre_keys)
                )
...
class FlatNeighbourFeatures(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        n_feat = SelectWithDefault(nidx, feat, 0.0)  # [V x K x F]
...
class RaggedGravNeteq(tf.keras.layers.Layer):
    def __init__(
        self,
        n_neighbours: int,
        n_dimensions: int,
        ...
        **kwargs,
        ...
        :param kwargs:
        """
        super(RaggedGravNeteq, self).__init__(**kwargs)
    ...
    def priv_call(self, inputs, training=None):
        ...
        # print("edge_feature before edge_mlp", edge_feature)
        edge_feature = self.edge_mlp(edge_feature)  # (BxNk)x128 (
        # print("WEIGHTS EDGE MLP")
        # tf.print(self.edge_mlp.weights)
        # print("edge_feature after mlp 1", edge_feature)
        edge_feature = self.edge_mlp2(edge_feature)
        # print("WEIGHTS EDGE MLP 2")
        # tf.print(self.edge_mlp2.weights)
        # print("edge_feature after mlp 2", edge_feature)
        cord_mlp = self.coord_mlp(edge_feature)
        # print("WEIGHTS CORD MLP ")
        ...
        (
            ...
            sdist,
            coord_diff,
        ) = self.compute_neighbours_and_distancesq(coods_new, row_splits, training)
        distancesq = tf.reshape(distancesq, [-1, self.n_neighbours])

        outfeats = self.create_output_features(h, neighbour_indices, distancesq)
        if self.return_self:
            neighbour_indices, distancesq = sidx, sdist

        return outfeats, coods_new, neighbour_indices, distancesq
