...
def _BinnedSelectKnn(
    ...
    return _binned_select_knn.BinnedSelectKnn(
        n_neighbours=K,
        coords=coords,
        bin_idx=bin_idx,
...
def BinnedSelectKnn(
    ...
    if n_bins is None:
        n_bins = tf.math.pow(
            tf.cast(elems_per_rs, dtype="float32") / (K / 32), 1 / max_bin_dims
        )
    ...
    dbinning, binning, nb, bin_width, nper = BinByCoordinates(
        bin_coords, row_splits, n_bins=n_bins, name=name
    )
    ...
    # add a leading 0
    bin_boundaries = tf.concat(
        [tf.zeros([1], dtype="int32"), nper], axis=0
    )  # row_splits[0:1]
    # make it row split like
    bin_boundaries = tf.cumsum(bin_boundaries)

    idx, dist = _BinnedSelectKnn(
        K,
        scoords,
        sbinning,
        sdbinning,
    ...
    dist = tf.where(idx < 0, 0.0, dist)  # safety
...
def _BinnedSelectKnnGrad(op, idxgrad, dstgrad):
    ...
    coord_grad = _sknn_grad_op.SelectKnnGrad(
        grad_distances=dstgrad, indices=indices, distances=distances, coordinates=coords
    )
