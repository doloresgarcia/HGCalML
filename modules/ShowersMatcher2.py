...
from modules.ShowersMatcher import calculate_iou_serial_fast
...
@njit
def _calculate_iou_serial_fast_comp(
    intersection_sum_matrix, pred_sum, truth_sum, truth_sid, pred_sid, hit_weight
):
    if not (len(truth_sid) == len(pred_sid) == len(hit_weight)):
        raise RuntimeError(
            "Length of truth_sid, pred_sid and hig_weight must be the same"
        )
...
def x(
    truth_sid, pred_sid, truth_shower_sid, pred_shower_sid, hit_weight, return_all=False
):
    ...
    _calculate_iou_serial_fast_comp(
        intersection_matrix, pred_sum, truth_sum, truth_sid_2, pred_sid_2, hit_weight
    )
    ...
    union_matrix = (
        pred_sum[:, np.newaxis] + truth_sum[np.newaxis, :] - intersection_matrix
    )
...
class ShowersMatcher:
    ...
    def _build_data_graph(self):
        ...
        pred_sid = np.where(
            pred_sid == -1, -1, pred_sid + offset
        )  # Offset node ids for pred nodes, can also be +1.
        self.pred_sid = pred_sid
        pred_shower_sid = [
            pred_sid[x, 0] for x in self.pred_alpha_idx if pred_sid[x] != -1
        ]
        ...
        for i in range(len(pred_shower_sid)):
            ...
            for k in keys:
                if k in skip:
                    ...
                    # import pdb
                    # pdb.set_trace()
                    node_attributes[k] = self.predictions_dict[k][
                        self.pred_alpha_idx[i]
                    ]
                    # Same as 'else' should work.
                else:
                    # print("X", k)
                    node_attributes[k] = self.predictions_dict[k][
                        self.pred_alpha_idx[i], 0
                    ]
    ...
    def _cost_matrix_angle_based(self, truth_shower_sid, pred_shower_sid):
        pred_shower_energy = [
            self.graph.nodes[x]["dep_energy"] for x in pred_shower_sid
        ]
        ...
        if self.de_e_cut == -1:
            ...
            allow = (
                lambda i, j: (
                    np.abs(pred_shower_energy[i] - truth_shower_energy[j])
                    / truth_shower_energy[j]
                )
                < de_e_cut_on_matching
            )
        ...
        for a, j in enumerate(pred_shower_sid):
            ...
            for b, i in enumerate(truth_shower_sid):
                ...
                if angle(x, y) < self.angle_cut and allow(x, y):
                    C[a, b] = min(x["energy"], y["energy"])

        return C

    def _cost_matrix_intersection_based(
        self, truth_shower_sid, pred_shower_sid, return_raw=False
    ):
        pred_shower_energy = np.array(
            [self.graph.nodes[x]["pred_energy"] for x in pred_shower_sid]
        )
        truth_shower_energy = np.array(
            [self.graph.nodes[x]["truthHitAssignedEnergies"] for x in truth_shower_sid]
        )
        weight = self.features_dict["recHitEnergy"][:, 0]

        iou_matrix = calculate_iou_serial_fast(
            self.truth_dict["truthHitAssignementIdx"][:, 0],
            self.pred_sid,
            truth_shower_sid,
            pred_shower_sid,
            weight,
        ...
        # 0/0
        secondary_condition = np.ones(
            (len(pred_shower_energy), len(truth_shower_energy)), np.bool
        )
        ...
        if self.de_e_cut != -1:
            c2 = (
                np.abs(
                    pred_shower_energy[:, np.newaxis]
                    - truth_shower_energy[np.newaxis, :]
                )
                / -truth_shower_energy[np.newaxis, :]
                < self.de_e_cut
            )
        ...
        if self.match_mode == "iou_max":
            ...
            C_s = np.min(
                pred_shower_energy[:, np.newaxis], truth_shower_energy[np.newaxis, :]
            )
            C_s[iou_matrix < self.iou_threshold] = 0
            C_s[np.logical_not(secondary_condition)] = 0
            C[0 : len(pred_shower_sid), 0 : len(truth_shower_sid)] = C_s
        ...
        for p, t in zip(row_id, col_id):
            if C[p, t] > 0:
                matched_full_graph.add_edge(
                    truth_shower_sid[t], pred_shower_sid[p], attached_in_pass=0
                )
    ...
    def get_result_as_dataframe(self):
        event_variables = (
            None  # Not sure if it should be a dataframe, can just be a dictionary
        )
        skip_keys = {"type"}
