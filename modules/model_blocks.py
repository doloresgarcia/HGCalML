...
>>>>>>> 0f27541 (several bugfixes, rsu-model runs now since 10 minutes)
    """
...
def condition_input(orig_inputs, no_scaling=False, no_prime=None):
    ...
    if not "coords" in orig_inputs.keys():
        ...
        orig_inputs["no_coords"] = SelectFeatures(0, 4)(processed_features)
...
def tiny_pc_pool(
    ...
    x_e = CreateGraphCondensationEdges2(name=name + "_gc_edges")(
        x_e, trans_a
    )  # V x 5+1 x 15

    x_e = Dense(
        dmp_compress // 2,
        name=name + "_graphn_edges_d1",
        activation="elu",
        trainable=trainable,
    )(x_e)
    x_e = Dense(
        8, name=name + "_graphn_edges_d2", activation="elu", trainable=trainable
    )(x_e)
    ...
    x_e = Dense(
        12 * (K_gp + 1),
        name=name + "_graphn_edges_d3",
        activation="elu",
        trainable=trainable,
    )(x_e)
    x_e = Dense(
        8 * (K_gp + 1),
        name=name + "_graphn_edges_d4",
        activation="elu",
        trainable=trainable,
    )(x_e)
    x_e = Reshape([K_gp + 1, 8])(x_e)
    x_e = Dense(
        1, name=name + "_graphn_edges_final", activation="sigmoid", trainable=trainable
    )(x_e)
    x_e = Multiply()(
        [x_e, GetNeighbourMask()(trans_a)]
    )  # mask out non existing ones right away
