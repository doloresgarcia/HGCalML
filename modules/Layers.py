...
global_layers_list[
    "LLFullObjectCondensationUncertainty"
] = LLFullObjectCondensationUncertainty
...
from Regularizers import (
    OffDiagonalRegularizer,
    WarpRegularizer,
    AverageDistanceRegularizer,
    MeanMaxDistanceRegularizer,
)
...
def layernorm(x, return_norm=False):
    ...
    if return_norm:
        x = tf.concat(
            [x / norm * tf.sqrt(tf.cast(x.shape[-1], "float32")), norm], axis=-1
        )
    else:
...
class InputNormalization(Layer):
    def __init__(self, multipliers, biases, **kwargs):
...
class CreateZeroMask(Layer):
    ...
    def call(self, inputs):
        ...
        mask = tf.where(inputs[:, :, self.feature_index] > 0, zeros + 1.0, zeros)
...
class AveragePoolVertices(Layer):
    ...
    def call(self, inputs):
        # create mask
        n_nonmasked = tf.cast(
            tf.count_nonzero(tf.reduce_sum(inputs, axis=2), axis=-1), dtype="float32"
        )
...
class TransformCoordinates(Layer):
    def __init__(
        self, feature_index_x=0, feature_index_y=1, feature_index_z=2, **kwargs
    ):
    ...
    def get_config(self):
        config = {
            "feature_index_x": self.feature_index_x,
            "feature_index_y": self.feature_index_y,
            "feature_index_z": self.feature_index_z,
        }
...
class SortPredictionByEta(Layer):
    ...
    def call(self, inputs):
        ...
        energies = inputs[1][
            :, :, self.input_energy_index : self.input_energy_index + 1
        ]
        ...
        weighted_etas = tf.reduce_sum(pred_energies * etas, axis=1) / (
            pred_sumenergy + K.epsilon()
        )
        weighted_etas = tf.where(
            tf.abs(weighted_etas) > 0.1,
            weighted_etas,
            tf.zeros_like(weighted_etas) + 500.0,
        )
        # B x Fracs

        ranked_etas, ranked_indices = tf.nn.top_k(
            -weighted_etas, tf.shape(pred_sumenergy)[1]
        )
        ...
        batch_indices = tf.tile(
            batch_range, [1, tf.shape(predicted_fracs)[2], 1]
        )  # B x Fracs x 1
        ...
        identity_matrix = tf.tile(
            identity_matrix, [tf.shape(pred_sumenergy)[0], 1, 1]
        )  # B x F x F
        ...
        sorted_identity_matrix = tf.tile(
            sorted_identity_matrix, [1, tf.shape(predicted_fracs)[1], 1, 1]
        )
        # B x V x Fm x Fm
        ...
        return tf.squeeze(
            tf.matmul(sorted_identity_matrix, sorted_predicted_fractions), axis=-1
        )

    def get_config(self):
        config = {
            "input_energy_index": self.input_energy_index,
...
class RobustModel(tf.keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        if submodel is not None:
            ...
            self.model = tf.keras.Model().from_config(
                submodel, custom_objects=custom_objects
            )
        else:
            self.outputs_keys = [x[0] for x in model_outputs]
            outputs_placeholders = [x[1] for x in model_outputs]
            self.model = tf.keras.Model(
                inputs=model_inputs, outputs=outputs_placeholders
            )
    ...
    def get_config(self):
        config = {
            "skip_non_finite": self.skip_non_finite,
    ...
    def train_step(self, data):
        ...
        if is_valid:
            gradients = tape.gradient(loss, trainable_vars)
            is_valid = is_valid and not bool(
                tf.reduce_any([_grad is None for _grad in gradients])
            )

        if is_valid:
            num_non_finite_tensors = float(
                tf.reduce_sum(
                    tf.cast(
                        [
                            tf.reduce_any(tf.math.logical_not(tf.math.is_finite(_grad)))
...
class DictModel(tf.keras.Model):
    def __init__(self, inputs, outputs: dict, *args, **kwargs):  # force to be dict
        """
