...
from Layers import RaggedGravNet, ScaledGooeyBatchNorm2, RaggedGravNeteq
from Regularizers import AverageDistanceRegularizer
...
def gravnet_model(
    ...
    x = ScaledGooeyBatchNorm2(fluidity_decay=0.01)([h, is_track])

    x_in = ScaledGooeyBatchNorm2(fluidity_decay=0.01, invert_condition=True)(
        [x, is_track]
    )
    ...
    for i in range(TOTAL_ITERATIONS):
        ...
        xgn, gncoords, gnnidx, gndist = RaggedGravNeteq(
...
model, history = train.trainModel(
    nepochs=200, batchsize=NBATCH, additional_callbacks=cb
)
