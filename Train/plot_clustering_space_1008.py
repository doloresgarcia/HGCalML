...
IN_DIR = "/eos/user/g/gkrzmanc/ds2507results_bk/philip-dataset-140823"
...
for file in os.listdir(IN_DIR):
    if file.startswith("KERAS_check_model_block") and file.endswith(".h5"):
        # KERAS_check_model_block_0_epoch_08.h5
        print(file)
        block = int(file.split("_")[4])
        epoch = int(file.split("_")[6].split(".")[0])
        id = (block, epoch)
        assert id not in files
        files[id] = file
