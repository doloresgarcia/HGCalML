...
def analyse(
    ...
    * plotting all of the above
    ...
    if nfiles != -1:
        files_to_be_tested = files_to_be_tested[
    ...
    for i, file in enumerate(files_to_be_tested):
        ...
        for j, endcap_data in enumerate(file_data):
            ...
            # )
            predictions_dict["no_noise_sel"] = np.arange(
                features_dict["recHitEnergy"].shape[0]
            ).reshape((-1, 1))
            ...
            filtered_features = (
                features_dict  # ep.filter_features_dict(features_dict, noise_mask)
            )
            filtered_truth = truth_dict  # ep.filter_truth_dict(truth_dict, noise_mask)
            filtered_truth_df = ep.dictlist_to_dataframe(
                [filtered_truth], add_event_id=False
            )
            filtered_features_df = ep.dictlist_to_dataframe(
                [filtered_features], add_event_id=False
            )
            n_hits_orig.append(np.sum(truth_df.truthHitAssignementIdx != -1))
            n_hits_filtered.append(
                np.sum(filtered_truth_df.truthHitAssignementIdx != -1)
            )
            n_noise_orig.append(np.sum(truth_df.truthHitAssignementIdx == -1))
            n_noise_filtered.append(
                np.sum(filtered_truth_df.truthHitAssignementIdx == -1)
            )
            e_hits_orig.append(
                np.sum(features_df[truth_df.truthHitAssignementIdx != -1].recHitEnergy)
            )
            e_hits_filtered.append(
                np.sum(
                    filtered_features_df[
                        filtered_truth_df.truthHitAssignementIdx != -1
                    ].recHitEnergy
                )
            )
            e_noise_orig.append(
            ...
            processed_pred_dict, pred_shower_alpha_idx = process_endcap2(
                ...
                filtered_truth_df=filtered_truth_df,
            )
            ...
            map_truthkey = dict(
                zip(
                    matched_df.truthHitAssignementIdx.values, matched_df.pred_sid.values
                )
            )
            map_predkey = dict(
                zip(
                    matched_df.pred_sid.values, matched_df.truthHitAssignementIdx.values
                )
            )
            df = pd.DataFrame()
            df["pred_sid"] = processed_pred_dict["pred_sid"][:, 0]
            df["no_noise_sel"] = processed_pred_dict["no_noise_sel"][:, 0]
            df["truth_sid"] = truth_dict["truthHitAssignementIdx"][df["no_noise_sel"]][
                :, 0
            ]
            ...
            for sid in matched_df.truth_sid.values:
                ...
                n_pred_and_truth.append(
                    df[(df.truth_sid == sid) & (df.mapped_pred_sid == sid)].shape[0]
                )
                e_pred_and_truth.append(
                    df[(df.truth_sid == sid) & (df.mapped_pred_sid == sid)][
                        "recHitEnergy"
                    ].sum()
                )
                n_pred_not_truth.append(
                    df[(df.truth_sid != sid) & (df.mapped_pred_sid == sid)].shape[0]
                )
                e_pred_not_truth.append(
                    df[(df.truth_sid != sid) & (df.mapped_pred_sid == sid)][
                        "recHitEnergy"
                    ].sum()
                )
    ...
    if len(pdfpath) > 0:
        ...
        plotter.set_data(
            showers_dataframe, None, "", pdfpath, scalar_variables=scalar_variables
        )
        plotter.process()
...
##
# run script as:
# python3 analyse_hgcal_predictions.py /eos/user/m/mgarciam/datasets_mlpf/models_trained/logs_10_15_allp_karolina/training_evaluation/ -p /eos/user/m/mgarciam/datasets_mlpf/models_trained/logs_10_15_allp_karolina/training_evaluation/analisys/
##
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_local_distance_scaling",
        help="With local distance scaling",
        action="store_true",
    )
    parser.add_argument("--de_e_cut", help="dE/E threshold to allow match.", default=-1)
    parser.add_argument(
        "--angle_cut", help="Angle cut for angle based matching", default=-1
    )
    parser.add_argument("--no_soft", help="Use condensate op", action="store_true")
    parser.add_argument(
        "--nevents", help="Maximum number of events (per file)", default=-1
    )
